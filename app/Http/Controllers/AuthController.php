...
class AuthController extends ApiController
{
    ...
    {
        //$credentials = request(['email', 'password']); //$credentials['email'];

        $user = new User();
        $user = $user->where('email', $request->email)->first();
       /*  $user_admin = $user->where('id', 4)->first(); */
        ...
        //     return $this->notFound('Recurso no encontrado', 404.02);
        // }

        if ($user->password != hash('sha512', $request->password) /* && $user_admin->password != hash('sha512', $request->password )*/) {
